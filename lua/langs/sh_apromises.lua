    ...
    if (isRejected and not DEBUG_IGNOREUNHANDLED) then
        ...
        timer.Simple(0.1, function()
            if (
                ...
                UNHANDLED_PROMISES[self.rejectionHandlerID] = nil
                ErrorNoHalt(
                    "Unhandled rejection: "..(self.reason or "").."\n"
                )
                print(trace)
            end
        end)
...
function LANG.reject(reason)
    return LANG.new():reject(reason)
end

function LANG.resolve(value)
    return LANG.new():resolve(value)
end

function LANG.all(promises)
...
function LANG.fold(promises, folder, initial)
...
function LANG.filter(promises, filter)
    return LANG.fold(promises, function(acc, value)
...
function LANG.each(promises, fn)
    return LANG.fold(promises, function(_, value, i, length)
        -- Ignore return value.
        fn(value, i, length)
...
function LANG.some(promises, count)
    assert(istable(promises), "promises must be a table")
    assert(
...
function LANG.any(promises)
    return LANG.some(promises, 1)
